...
import { Flag, Globe, Calendar, Shuffle, Trophy, History, Trash2, Home } from "lucide-react";
...
export function HistoryMenu({ onSelectTopic }: HistoryMenuProps) {
  const { data: results, isLoading } = useResultsBySubject('history');
  const clearResults = useClearResultsBySubject();

  const topics = [
    { id: 'brazil' as const, label: 'Brasil', icon: Flag, color: 'bg-orange-500', shadow: 'shadow-orange-500/30', desc: 'Descobrimento à democracia' },
    { id: 'world' as const, label: 'Mundial', icon: Globe, color: 'bg-purple-500', shadow: 'shadow-purple-500/30', desc: 'Grandes civilizações e guerras' },
    { id: 'dates' as const, label: 'Datas', icon: Calendar, color: 'bg-red-500', shadow: 'shadow-red-500/30', desc: 'Datas históricas importantes' },
    ...
    <div className="w-full max-w-5xl mx-auto space-y-12">
      <div className="flex items-center gap-4">
        ...
        <div className="text-center flex-1">
          ...
          >
            <span className="text-orange-500">História</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            Aprenda sobre eventos que moldaram o mundo
          </motion.p>
        </div>
        <div className="w-10" />
      </div>
                  ...
                  <div key={result.id} className="flex items-center justify-between p-3 rounded-xl bg-white border border-border/50">
                    <div>
                      <p className="font-bold text-sm">
                        {result.topic ? topicLabels[result.topic as Topic] : 'História'}
                      </p>
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 gap-2">
            ...
            <Button
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                if (window.confirm('Zerar estatísticas de História?')) {
                  clearResults.mutate('history');
                }
              }}
